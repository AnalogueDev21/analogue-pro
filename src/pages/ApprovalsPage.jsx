...
import { ApprovalDecisionModal, ApprovalList, REQUEST_TYPES } from '@/features/approvals'
...
export default function ApprovalsPage() {
  ...
  const confirmDecision = async (comment) => {
    try {
      ...
      toast(decision.action === 'approved' ? 'Request approved' : 'Request rejected')
      setDecision(null)
    } catch (e) {
  ...
  return (
    <PermissionGate perms={['approval.view', 'approval.manage']} fallback={<EmptyState title="No access" />}>
      {ToastEl}
      <PageHeader
        title="Approvals"
        subtitle="Review and approve requests from leave, OT, transfers, and payroll"
        ...
        <div className="flex gap-1 bg-white border border-slate-100 rounded-2xl p-1 w-fit overflow-x-auto">
          {TABS.map(tab => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition-all ${status === tab ? 'bg-primary-700 text-white' : 'text-slate-500 hover:bg-slate-50'}`}
            >
              {tab}
            </button>
          ))}
