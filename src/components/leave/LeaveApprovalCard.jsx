  ...
  return (
    <Card>
      <div className="flex items-start gap-4 flex-wrap">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-bold text-slate-800 truncate">{l.employees?.first_name} {l.employees?.last_name}</p>
            <p className="text-xs text-slate-400 truncate">{l.employees?.departments?.name} / {l.employees?.positions?.name}</p>
          </div>
        </div>
      ...
      <div className="mt-3 rounded-xl border border-slate-100 bg-slate-50 p-3">
        <p className="text-xs font-semibold text-slate-400 mb-1">{t('leave.reason')}</p>
        <p className="text-sm leading-6 text-slate-700 whitespace-pre-wrap break-words">
          {l.reason || choose(i18n, 'ไม่ได้ระบุเหตุผล', 'No reason provided')}
        </p>
      </div>
      ...
      {l.reject_reason && (
        <div className="mt-3 rounded-xl border border-red-100 bg-red-50 p-3">
          ...
          <p className="text-sm leading-6 text-red-700 whitespace-pre-wrap break-words">{l.reject_reason}</p>
        </div>
      )}
